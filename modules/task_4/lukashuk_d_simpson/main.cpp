...
double func_3(const std::vector<double>& x) {
    return (std::exp(x[0] / x[1]) + x[2] * x[3] * std::sin(x[1]) / std::cos(x[2]))
           / std::exp(x[3]);
}

double func_4(const std::vector<double>& x) {
    return (std::exp(x[0] / x[1] / x[2]) - x[3] * x[4] * std::sin(x[1]) / std::cos(x[2])
...
bool test(const std::vector<int>& steps, const FUNC_T& f) {
    ...
    std::cout << "Seq: " << static_cast<double>(seq_time.count()) /
              10e+8 << '\n';
    std::cout << "Par: " << static_cast<double>(par_time.count()) /
              10e+8 << '\n';
