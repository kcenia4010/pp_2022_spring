...
#include "../../../modules/task_3/krapivenskikh_s_conjugate_gradient/conjugate_gradient.h"
...
std::vector<double> multiMtxVecSeq(std::vector<std::vector<double> > mtx, std::vector<double> x) {
...
std::vector<double> multiMtxVecPar(std::vector<std::vector<double> > mtx, std::vector<double> x) {
    assert(mtx.size() > 0 && x.size() > 0);
    assert(mtx.size() % x.size() == 0);
    std::vector<double> res(x);
    int n = mtx.size();
...
std::vector<double> calculateResSeq(std::vector<std::vector<double> > mtx, const std::vector<double>& b) {
    int m = mtx[0].size();
    assert(m > 0);
    ...
    r0 = subVec(b, multiMtxVecSeq(mtx, x0));
    z0 = r0;
    int i = 0;
    do {
        i++;
        double alpha = (scalarProduct(r0, r0) / (scalarProduct(multiMtxVecSeq(mtx, z0), z0)));
...
std::vector<double> calculateResPar(std::vector<std::vector<double> > mtx, const std::vector<double>& b) {
    int m = mtx[0].size();
    assert(m > 0);
    ...
    r0 = subVec(b, multiMtxVecPar(mtx, x0));
    z0 = r0;
    int i = 0;
    do {
        i++;
        double alpha = (scalarProduct(r0, r0) / (scalarProduct(multiMtxVecPar(mtx, z0), z0)));
        std::vector<double> x = sumVec(x0, multiVec(alpha, z0));
